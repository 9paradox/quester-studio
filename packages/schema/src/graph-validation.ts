...
export function validateFlowGraph(flow: FlowV1): FlowGraphValidationResult {
  ...
  for (const edge of flow.edges) {
    if (!nodeById.has(edge.source)) {
      issues.push({ path: `edges/${edge.id}`, message: `Unknown source node: ${edge.source}` });
    }
    if (!nodeById.has(edge.target)) {
      issues.push({ path: `edges/${edge.id}`, message: `Unknown target node: ${edge.target}` });
    }
  }

  const inputNodes = flow.nodes.filter((n) => n.type === "input");
  if (inputNodes.length === 0) {
    issues.push({ path: "nodes", message: "Flow must contain at least one input node" });
  }
