...
- [ ] **X** — During a run, nodes animate in-progress / success / fail without obvious canvas lag.
- [ ] **Y** — Run status panel lists all nodes with order, state, duration; every assert shows pass/fail + message.
- [ ] **Z** — Final overall status (success / failed / cancelled + counts) matches engine; Response/Logs stay per-flow isolated.
